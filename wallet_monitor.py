...
2. 手动代币配置 - 支持手动配置常见代币作为备用查询方案
3. 多链支持 - 支持Ethereum、Polygon、Arbitrum、Optimism、Base等主流EVM链
4. 智能私钥识别 - 自动识别EVM私钥类型
5. 智能去重 - 自动去重避免重复查询同一代币
6. 可配置查询限制 - 可设置每个链最多查询的代币数量
    ...
    def _validate_addresses(self):
        ...
        try:
            ...
            if not Web3.is_address(self.EVM_TARGET_ADDRESS):
                print(f"⚠️ 无效的目标地址: {self.EVM_TARGET_ADDRESS}")
...
def identify_private_key_type(private_key: str) -> str:
    ...
    try:
        ...
        logger.warning(f"无效的私钥格式，私钥长度: {len(cleaned_key)}")
        return "unknown"
...
class WalletMonitor:
    ...
    def collect_private_keys(self):
        """收集私钥"""
        print("\n" + "="*60)
        print("🔑 请输入私钥（一行一个，支持EVM格式）")
        print("📝 程序会自动识别EVM私钥类型")
    ...
    async def filter_addresses_with_history(self):
        ...
        for i, address in enumerate(self.addresses):
            ...
            if self.addr_type[address] == "evm":
                ...
                # 不支持的地址类型
                logger.warning(f"不支持的地址类型: {self.addr_type[address]}")
                active_chains = []
            ...
            for key_info in self.private_keys:
                try:
                    if key_info["type"] == "evm":
                        ...
                        logger.warning(f"⚠️ 不支持的私钥类型: {key_info['type']}，跳过私钥")
                        continue
    ...
    def _rebuild_chain_mapping(self, saved_mapping: dict) -> bool:
        ...
        try:
            ...
            for client in getattr(self, 'evm_clients', []):
                chain_lookup[client["chain_id"]] = client
                chain_lookup[client["name"]] = client
            ...
            for addr, chain_info_list in saved_mapping.items():
                ...
                for chain_info in chain_info_list:
                    ...
                    if isinstance(chain_info, str):
                        ...
                        client = chain_lookup.get(chain_name)
                    else:
                        # 新格式（包含详细信息）
                        ...
                        if chain_type == "evm":
                            ...
                            client = chain_lookup.get(chain_id) or chain_lookup.get(chain_name)
                        else:
                            chain_name = chain_info.get("name")
                            # 不支持的链类型
                            client = None
...
║  💎 EVM 全生态支持 | 🛡️ 多重安全保护 | ⚡ 实时余额监控                        ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  🌟 特色功能:                                                                 ║
║  • 🔍 50+ EVM链支持       • 🪙 ERC-20代币支持                              ║
║  • 🛡️ 智能安全验证        • 🔄 自动RPC故障转移                              ║
║  • 📱 Telegram实时通知     • 💾 加密状态存储                                 ║
║  • 🎨 彩色终端界面         • 📊 详细监控统计                                 ║
╚══════════════════════════════════════════════════════════════════════════════╝
{Style.RESET_ALL}"""
        ...
        # 链连接统计
        chain_count = len(getattr(self, 'evm_clients', []))
        ...
        print(f"└─ ⛓️  已连接链: {Fore.BLUE}{chain_count}{Style.RESET_ALL} 条")
    ...
    def show_control_menu(self):
        ...
        while True:
            ...
            print(f"\n{Fore.WHITE}{Back.MAGENTA} 🎛️  控制中心 {Style.RESET_ALL}")
            print("="*70)
            print("🔧 操作选项:")
            print("1. 📝 添加钱包私钥")
            print("2. 📋 查看当前地址")
            ...
            if choice == "1":
                self.add_new_address_simple()
            elif choice == "2":
                self.list_all_addresses_simple()
            elif choice == "3":
                self.reinitialize_system()
            elif choice == "4":
                self.start_monitoring_simple()
            elif choice == "5":
                self.stop_monitoring_simple()
            elif choice == "6":
                self.save_state_with_feedback()
            elif choice == "7":
                self.load_state_simple()
            elif choice == "8":
                self.check_connections_simple()
            elif choice == "9":
    ...
    def add_new_address_simple(self):
        """简化的添加地址功能"""
        ...
        try:
            ...
            if key_type == "evm":
                ...
                # 检查地址是否已存在
                if address in getattr(self, 'addresses', []):
                    print(f"{Fore.YELLOW}⚠️ 地址 {address} 已存在{Style.RESET_ALL}")
                    input("按回车键继续...")
                    return
                ...
                # 添加到列表
                self.addresses.append(address)
                self.addr_to_key[address] = {"key": private_key, "type": key_type}
                self.addr_type[address] = key_type
                
                print(f"{Fore.GREEN}✅ 成功添加地址: {address}{Style.RESET_ALL}")
                print(f"地址类型: {key_type.upper()}")
                ...
                print(f"{Fore.RED}❌ 不支持的私钥类型: {key_type}{Style.RESET_ALL}")
    ...
    def list_all_addresses_simple(self):
        """简化的地址列表显示"""
        print(f"\n{Fore.CYAN}📋 当前地址列表{Style.RESET_ALL}")
        print("="*50)
        ...
        for i, address in enumerate(self.addresses, 1):
            ...
            status = f"{Fore.GREEN}活跃{Style.RESET_ALL}" if is_active else f"{Fore.GRAY}非活跃{Style.RESET_ALL}"
            
            print(f"{i}. {address}")
            print(f"   类型: {addr_type.upper()} | 状态: {status}")
            
            if is_active:
                chains = self.active_addr_to_chains[address]
                print(f"   监控链: {len(chains)} 条")
            print()
    ...
    def reinitialize_system(self):
        """重新初始化系统"""
        print(f"\n{Fore.CYAN}🔧 重新初始化系统{Style.RESET_ALL}")
        print("="*50)
        
        print("正在重新初始化链客户端...")
        if self.initialize_evm_clients():
            print(f"{Fore.GREEN}✅ 系统重新初始化成功{Style.RESET_ALL}")
        else:
            print(f"{Fore.RED}❌ 系统重新初始化失败{Style.RESET_ALL}")
        
        input("按回车键继续...")
    
    def start_monitoring_simple(self):
        """简化的开始监控"""
        ...
        if not hasattr(self, 'addresses') or not self.addresses:
            print(f"{Fore.RED}❌ 请先添加地址{Style.RESET_ALL}")
            input("按回车键继续...")
            return
        ...
        if not hasattr(self, 'active_addr_to_chains') or not self.active_addr_to_chains:
            ...
            loop.run_until_complete(self.filter_addresses_with_history())
            
            if not self.active_addr_to_chains:
                print(f"{Fore.RED}❌ 没有活跃地址可监控{Style.RESET_ALL}")
                input("按回车键继续...")
                return
        ...
        monitor_thread = threading.Thread(target=start_monitoring_thread, daemon=False)
        monitor_thread.start()
        
        print(f"{Fore.GREEN}✅ 监控已启动{Style.RESET_ALL}")
        input("按回车键继续...")
    
    def stop_monitoring_simple(self):
        """简化的停止监控"""
    ...
    def load_state_simple(self):
        """简化的加载状态"""
        print(f"\n{Fore.CYAN}📂 加载状态{Style.RESET_ALL}")
        print("="*50)
        
        print("正在加载状态...")
        if self.load_state():
            print(f"{Fore.GREEN}✅ 状态加载成功{Style.RESET_ALL}")
            print(f"已加载 {len(getattr(self, 'addresses', []))} 个地址")
        else:
            print(f"{Fore.RED}❌ 状态加载失败{Style.RESET_ALL}")
        
        input("按回车键继续...")
    
    def check_connections_simple(self):
        """简化的连接检查"""
        print(f"\n{Fore.CYAN}🔍 检查连接{Style.RESET_ALL}")
        print("="*50)
        
        if hasattr(self, 'evm_clients') and self.evm_clients:
            working_count = 0
            total_count = len(self.evm_clients)
            ...
            for client in self.evm_clients:
                try:
                    if client['w3'].is_connected():
                        ...
                        gas_price = Web3.from_wei(client['w3'].eth.gas_price, 'gwei')
                        print(f"✅ {client['name']}")
                        print(f"   区块: {block_number:,}")
                        print(f"   Gas: {gas_price:.1f} Gwei")
                        print(f"   RPC: {client['rpc_type']}")
                    else:
                        print(f"❌ {client['name']} - 断开")
                except Exception as e:
                    print(f"❌ {client['name']} - 错误: {str(e)}")
                print()
            
            print(f"连接统计: {working_count}/{total_count} 正常")
            if working_count < total_count:
                print(f"{Fore.YELLOW}⚠️ 建议重新初始化系统{Style.RESET_ALL}")
        else:
            print(f"{Fore.RED}❌ 没有可用的客户端{Style.RESET_ALL}")
    ...
    def check_rpc_connections(self):
        ...
        # 其他链暂不支持
        print(f"\n{Fore.RED}❌ 其他链暂不支持{Style.RESET_ALL}")
    ...
    if not resume:
        ...
        for i, key_info in enumerate(monitor.private_keys, 1):
            ...
            if key_info["type"] == "evm":
                ...
                print(f"{Fore.RED}❌ 不支持的私钥类型: {key_info['type']}{Style.RESET_ALL}")
                continue
