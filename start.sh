...
check_dependencies() {
    ...
    if [ ${#missing_packages[@]} -ne 0 ]; then
        ...
        if [ ${#failed_packages[@]} -ne 0 ]; then
            echo -e "\n${RED}${CROSSMARK} 以下包安装失败: ${failed_packages[*]}${NC}"
            ...
            echo -e "\n${GREEN}${CHECKMARK} 所有依赖包安装完成${NC}"
        fi
    else
        echo -e "${GREEN}${CHECKMARK} 所有依赖包已满足${NC}"
    fi
